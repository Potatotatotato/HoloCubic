...
{
	ssid = DEFAULT_SSID;
	password = DEFAYLT_PASSWORD;
	cityWeatherData.cityName = DEFAULT_CITY;
	weather_apply_id = DEFAULT_WEATHER_APPLY_ID;
	weather_apply_secret = DEFAULT_WEATHER_APPLY_SECRET;
	weather_apply_id_and_secret = "&appid=" + weather_apply_id + "&appsecret=" + weather_apply_secret;
...
void SdCard::readConfigFile(void)
{
	ssid = tfCard.readFileLine("/config.txt", 1);	  // line-1 for WiFi ssid
	password = tfCard.readFileLine("/config.txt", 2); // line-2 for WiFi password

	cityWeatherData.cityName = tfCard.readFileLine("/config.txt", 3);

	weather_apply_id = tfCard.readFileLine("/config.txt", 6);
	weather_apply_secret = tfCard.readFileLine("/config.txt", 8);
	weather_apply_id_and_secret = "&appid=" + weather_apply_id + "&appsecret=" + weather_apply_secret;
}
